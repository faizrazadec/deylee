...
- **An account is required to start tracking, and required once.** Pressing Start
  while signed out raises sign-in and does not open a day; dismissing it leaves the
  timer where it was. This changed deliberately — earlier builds let a dismissed
  sign-in start the day, and the changelog for those versions says so.

  What did *not* change is local-first, and the distinction is the whole design. The
  gate is a stored session, never a live request: once signed in, the app opens days,
  counts work and closes them with no network at all, writing to SQLite exactly as
  before. An account is asked for once, not asked for continuously. A timer that
  stopped working on a train would still be worse than one that never synced.
