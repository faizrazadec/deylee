...
final class HistoryModel {
    ...
    func export(_ format: HistoryExportFormat) {
        ...
        panel.title = "Export Deylee data"
        panel.nameFieldStringValue = "deylee-\(range.from)_to_\(range.to).\(format.rawValue)"
    ...
    ///
    /// DeyleeKit's errors conform to `DeyleeError`, so `localizedDescription` returns
