...
/// What Dayly knows about a newer version of itself.
///
...
final class SettingsModel {
    ...
    /// Squirrel refuses to install a bundle that is not signed and notarised, and Dayly
    ...
    private static let releasesURLString = "https://github.com/faizrazadec/dayly/releases"
    ...
    /// `PreferenceKey` protocol of its own and the `DaylyKit` module name is shadowed by
    ...
    private func report(_ error: Error) {
        NSLog("[dayly] \((error as? PreferenceWriteError)?.message ?? "\(error)")")
        prefs = store.getAll()
        flash(.failed)
    }
    ...
    func backUpDatabase() {
        ...
        panel.title = "Back up Dayly data"
        panel.nameFieldStringValue = "dayly-backup-\(todayKey()).sqlite"
...
struct SettingsView: View {
    ...
    private var general: some View {
        SettingsSectionCard(
            title: "General",
            description: "How Dayly starts up and how it looks."
        ) {
            SettingsToggleRow(
    ...
    private var dataActions: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            .buttonStyle(DaylyButtonStyle(variant: .secondary, size: .medium))
...
/// promises that Dayly does the thing itself, which is the only signal an unsigned
