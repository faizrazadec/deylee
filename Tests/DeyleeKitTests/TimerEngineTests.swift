...
private final class Harness {
    ...
    init(now: EpochMs, dailyTargetHours: Double = 8) throws {
        path = NSTemporaryDirectory() + "deylee-engine-\(UUID().uuidString).sqlite"
...
@Suite @MainActor struct TimerEnginePersistence {
    @Test func stateSurvivesAnEngineRestart() throws {
        let path = NSTemporaryDirectory() + "deylee-restart-\(UUID().uuidString).sqlite"
        defer {
            for suffix in ["", "-wal", "-shm"] {
                try? FileManager.default.removeItem(atPath: path + suffix)
            }
    ...
    @Test func migrationsAreIdempotent() throws {
        let path = NSTemporaryDirectory() + "deylee-migrate-\(UUID().uuidString).sqlite"
        defer {
            for suffix in ["", "-wal", "-shm"] {
                try? FileManager.default.removeItem(atPath: path + suffix)
            }
    ...
    @Test func aFileFromANewerBuildIsRefusedBeforeAnyWrite() throws {
        let path = NSTemporaryDirectory() + "deylee-newer-\(UUID().uuidString).sqlite"
        defer {
            for suffix in ["", "-wal", "-shm"] {
                try? FileManager.default.removeItem(atPath: path + suffix)
            }
