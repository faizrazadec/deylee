...
/// Where Dayly's data lives, and how the native app adopts what the Electron build
/// left behind.
///
/// The Electron build stored everything in `~/Library/Application Support/dayly/`
/// (lower-case — Electron derived the folder from the package name, not the product
/// name). The native app keeps using that same folder and the same `dayly.sqlite`
...
public enum DataStore {
    public static let databaseFileName = "dayly.sqlite"
