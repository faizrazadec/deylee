...
final class PromptQueue {
    ...
    private func advance(_ apply: () throws -> Void) {
        ...
        do {
            ...
            NSLog("[dayly] prompt resolution failed: \(error)")
        }
    }
}
...
struct RecoveryPromptView: View {
    ...
    var body: some View {
        DaylyModal(title: "Unfinished session") {
    ...
    private var description: some View {
        (
            Text("Dayly closed while a \(prompt.segment.type.rawValue) segment was still running. It started at ")
            ...
            ForEach(options, id: \.choice.rawValue) { option in
                VStack(alignment: .leading, spacing: Space.xs) {
                    Button(option.label) { onResolve(option.choice) }
                        .buttonStyle(DaylyButtonStyle(variant: option.variant, fillWidth: true))
                        .disabled(busy)
                    Text(option.hint)
                        .font(Type.meta)
                        .foregroundStyle(Palette.fgFaint)
...
struct IdlePromptView: View {
    ...
    var body: some View {
        DaylyModal(title: "Away from your desk?") {
    ...
    var body: some View {
        DaylyModal(title: "Welcome back") {
            ...
            Button("Count as break") { onResolve(.countAsBreak) }
                .buttonStyle(DaylyButtonStyle(variant: .secondary))
                .disabled(busy)
...
struct EndDayConfirmView: View {
    ...
    var body: some View {
        DaylyModal(title: "End the day?", onDismiss: onCancel) {
            (
...
extension View {
    ...
    func daylyPrompts(_ model: AppModel) -> some View {
